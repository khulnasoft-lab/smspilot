...
<head>
    ...
    <link rel="icon" href="{site_url}/templates/_apidoc/img/favicon.ico">

    <title>Loading...</title>

    <link rel="stylesheet" href="{site_url}/templates/_apidoc/libs/bootstrap.min.css">
    <link rel="stylesheet" href="{site_url}/templates/_apidoc/libs/prettify.css">
    <link rel="stylesheet" href="{site_url}/templates/_apidoc/css/style.css">

    <script src="{site_url}/templates/_assets/js/libs/iframeResizer.contentWindow.min.js"></script>
    
    <script>
        window.site_name = "{system_site_name}";
        window.site_url = "{$site_url}";
        window.api_template = "{site_url}/templates/_apidoc";
    </script>
</head>
...
<body class="container">
    ...
    <script id="template-article" type="text/x-handlebars-template">
        <article id="api-{{article.group}}-{{article.name}}-{{article.version}}" {{#if hidden}}class="hide" {{/if}} data-group="{{article.group}}" data-name="{{article.name}}" data-version="{{article.version}}">
            ...
            {{#if article.author}}
            <h4 class="muted">Authored by: {{article.author}}</h4>{{/if}} {{#if article.deprecated}}
    ...
    <script id="template-article-compare-permission" type="text/x-handlebars-template">
        <p>
            ...
            {{#unless _last}}, {{/unless}} {{/if}} {{/if}} {{#if typeIns}}
            ...
            {{#unless _last}}, {{/unless}} {{/if}} {{/if}} {{else}} {{#if typeSame}} {{compare.name}} {{#if compare.title}}
    ...
    <script id="template-article-compare-param-block-body" type="text/x-handlebars-template">
        <tbody>
            ...
            <tr>
                <td class="code">
                    {{{splitFill source.field "." "&nbsp;&nbsp;"}}} {{#if source.optional}} {{#if compare.optional}} <span class="label label-optional">{{__ "optional"}}</span> {{else}} <span class="label label-optional label-ins">{{__ "optional"}}</span> {{/if}} {{else}} {{#if compare.optional}} <span class="label label-optional label-del">{{__ "optional"}}</span>{{/if}} {{/if}}
                </td>
    ...
    <script src="{site_url}/templates/_apidoc/libs/polyfill.js"></script>
    <script data-main="{site_url}/templates/_apidoc/main.js" src="{site_url}/templates/_apidoc/libs/require.min.js"></script>
</body>
